...
local function should_stage(filepath, patterns)
  ...
  for _, pattern in ipairs(Config.EXCLUDE_PATTERNS or {}) do
    if filepath == pattern then return false end
    if pattern:find "%*" then
      local regex = pattern:gsub("%.", "%."):gsub("%*", ".*") .. "$"
      if filepath:match(regex) then return false end
    end
    if pattern:sub(-1) == "/" and filepath:match("^" .. vim.pesc(pattern)) then return false end
  end
  ...
  for _, pattern in ipairs(patterns) do
    ...
    elseif p:find "%*" then
      ...
      if filepath:match(regex) then matches = not is_exclude end
    elseif p:sub(-1) == "/" and filepath:match("^" .. vim.pesc(p)) then
      matches = not is_exclude
    end
  end
...
---@return string
local function get_backup_dir() return vim.fn.expand(M.config.backup):gsub("/+$", "") end
...
local function init_backup_repo()
  ...
  if result[1] and result[1]:match "^%.git$" then
  ...
  if vim.v.shell_error ~= 0 then return false, "git init failed" end
...
local function copy_to_backup(filepath)
  ...
  if not src_stat or src_stat.type ~= "file" then return false, "source file not found: " .. abs_path end
  ...
  if not mkdir_ok then return false, "mkdir failed: " .. vim.trim(tostring(mkdir_err)) end

  local copy_ok, copy_err = pcall(vim.loop.fs_copyfile, abs_path, backup_path)
  if not copy_ok then return false, "copy failed: " .. tostring(copy_err) end
...
function M.setup(opts)
  ...
  local callback = function() M._handle_write() end
...
function M._handle_write()
  ...
  if not M.should_stage(bufname) then return end

  -- Copy file to backup
  local success, copy_result = copy_to_backup(bufname)
  if not success then
    if M.config.notify.failure then Log.failure("Timecapsule: " .. copy_result) end
    return
  end

  if M.config.notify.success then Log.success("Timecapsule: backed up " .. bufname) end
  local backup_path = copy_result

  local repo_success, repo_err = init_backup_repo()
  if not repo_success then
    if M.config.notify.failure then Log.failure("Timecapsule: " .. repo_err) end
    return
  end
  ...
  if abs_path:find("^" .. vim.pesc(backup_dir) .. "/") then return end
  ...
  if vim.v.shell_error ~= 0 then
    if M.config.notify.failure then Log.failure "Timecapsule: add failed" end
    return
  end
  ...
  if not next(diff_result) then
    if M.config.notify.success then Log.success "Timecapsule: file not modified, skipping commit" end
    return
  end

  local message = M.config.message_format:gsub("{path}", function() return bufname end)

  -- Commit in backup repo
  vim.fn.system { "git", "-C", backup_dir, "commit", "-m", message, "--" }
  if vim.v.shell_error ~= 0 then
    if M.config.notify.failure then Log.failure "Timecapsule: git commit failed" end
    return
  end

  if M.config.notify.success then Log.success("Timecapsule: committed " .. message) end
  ...
  if M.config.push and M.config.push.enable then
    ...
    if not push_success then
      if M.config.notify.failure then Log.failure("Timecapsule: " .. push_err) end
    else
      if M.config.notify.success then Log.success("Timecapsule: pushed to " .. branch) end
    end
  end
end
