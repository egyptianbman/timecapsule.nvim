...
function M.validate(opts)
  ...
  local merged = vim.tbl_deep_extend("force", M.default_config, filter_nil(opts))

  if type(merged.enable) ~= "boolean" then error "enable must be a boolean" end
  if not merged.enable then return merged end
  if type(merged.message_format) ~= "string" then error "message_format must be a string" end

  if merged.file_patterns ~= nil and type(merged.file_patterns) ~= "table" then
    error "file_patterns must be a table or nil"
  end

  if merged.notify and type(merged.notify) ~= "table" then error "notify must be a table" end

  if not merged.backup or type(merged.backup) ~= "string" then error "backup must be a non-empty string" end

  if merged.push and type(merged.push) == "table" then
    if type(merged.push.enable) ~= "boolean" then error "push.enable must be a boolean" end
    if merged.push.branch then
      if type(merged.push.branch) ~= "string" then error "push.branch must be a string" end
    end
  end
