...
describe("Config system", function()
  ...
  it("should have default config values", function()
    ...
    assert.equal(Config.default_config.backup, vim.fn.stdpath "data" .. "/timecapsule")
  ...
  it("should error on invalid message_format", function()
    assert.error(function() Config.validate { message_format = 123 } end)
  end)

  it("should error on invalid push.enable", function()
    assert.error(function() Config.validate { push = { enable = "yes" } } end)
  end)

  it("should error on invalid push.branch", function()
    assert.error(function() Config.validate { push = { enable = true, branch = 123 } } end)
  end)

  it("should error on invalid notify type", function()
    assert.error(function() Config.validate { notify = "enabled" } end)
  end)

  it("should error on invalid file_patterns type", function()
    assert.error(function() Config.validate { file_patterns = "*.lua" } end)
  end)
...
describe("Main module", function()
  ...
  after_each(function()
    Helpers.cleanup "/tmp/tc_test_backup"
    Helpers.cleanup "/tmp/tc_test_source"
    reset_module()
  end)
end)
...
describe("init_backup_repo", function()
  ...
  it("should return false when git init fails", function()
    local backup_dir = tmp_base .. "/init_fail"
    Helpers.cleanup(backup_dir)
    -- Create a directory with a .git file (not dir) so git init fails
    vim.fn.mkdir(backup_dir, "p")
    vim.fn.writefile({ "" }, backup_dir .. "/.git")
    Timecapsule.config = { backup = backup_dir }

    local success, err = Timecapsule._init_backup_repo()
    assert.is_false(success)
    assert.truthy(err)
    assert.truthy(string.find(err, "git init failed"))
  end)
end)
...
describe("_handle_write error paths", function()
  ...
  it("should return early with empty buffer name", function()
    ...
    -- No backup should have been created for empty buffer
    assert.is_false(Helpers.dir_exists(backup_dir), "No backup dir should exist for empty buffer")
  ...
  it("should skip files inside the backup directory", function()
    ...
    --[[ File was created directly in the backup dir for this test. ]]
